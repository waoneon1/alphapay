 <!-- Subscribe Section -->
<div class="alp-subsc row alp-section">
	<div class="col-md-6 col-12 alp-subsc--left alp-row">
		<div>
			<h2>Mau Dapat Info Terbaru dari AlphaPay?</h2>
			<p>Daftarkan email dan nomor hp Anda pada kolom berikut untuk menerima berbagai info, tips bisnis, dan promo menarik dari AlphaPay.</p>
			<div class="alp-subsc--form alp-form">

				<div class="field">
					<input type="email" name="email" id="email" placeholder="Email">
					<label for="fullname">Email</label>
				</div>

				<div class="field">
					<input type="text" name="nohp" id="nohp" placeholder="Nomor HP">
					<label for="email">Nomor HP</label>
				</div>

    			<a class="alp-btn btn alp-btn-blue" href="">Daftarkan Saya</a>
			</div>
		</div>
	</div>
	<div class="col-md-6 col-12 alp-subsc--right d-none d-md-block">
		&nbsp;
	</div>
</div>